...
function watch() {
  browserSync.init({
    open: 'external',
    proxy: 'http://localhost:8888/demo',
    port: 8080
  });
  gulp.watch(styleWatchFiles, gulp.series([css, concatCSS]));
  // gulp.watch(jsSRC, javascript);
  // gulp.watch(imgSRC, imgmin);
  gulp.watch([jsdist + 'visionboard.js', scss + 'style.min.css']).on('change', browserSync.reload);
}
